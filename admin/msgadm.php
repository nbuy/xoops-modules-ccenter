...
if ( isset( $_POST['store'] ) ) {
	...
	if ( $res && $xoopsDB->getRowsNum( $res ) == 1 ) {
		...
		$log      = '';
		$invalues = array(
			'status'  => _AM_MSG_STATUS,
			'touid'   => _AM_MSG_CHARGE,
			'value'   => _AM_MSG_VALUE,
			'comment' => _AM_MSG_COMMS
		);
		foreach ( $invalues as $k => $lab ) {
			$v = in_array( $k, array(
				'touid',
				'value'
			) ) ? (int) $_POST[ $k ] : $myts->stripSlashesGPC( $_POST[ $k ] );
...
function msg_list() {
	...
	$labels = array(
		'mtime'  => _AM_FORM_MTIME,
		'status' => _AM_MSG_STATUS,
		'fidref' => _AM_FORM_TITLE,
		'cfrom'  => _AM_MSG_FROM,
		'uname'  => _AM_MSG_CHARGE,
		'comms'  => _AM_MSG_COMMS,
		'ope'    => _AM_OPERATION
	);
	$orders = array(
